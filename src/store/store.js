...
export default new Vuex.Store({
    ...
    getters: {
        getLeadhitSiteId: state => state.leadhitSiteId,
    ...
    actions: {
        ...
        async getData({commit}, id) {
            ...
            let response = await axios('https://track-api.leadhit.io/client/test_auth', {headers: headers});
            console.log(response)
            if (response.data.message === 'ok') commit('SAVE_ID', id);
        }
    },
    mutations: {
        SAVE_ID(state, id) {
